...
import pipeline   # scoring functions only — NOT update_history
...
def rescore(hist: pd.DataFrame) -> pd.DataFrame:
    """Replicate pipeline.update_history scoring block on existing hist DataFrame."""
...
def main():
    ...
    # ── Trim trailing NaN rows so pipeline doesn't think they exist ───────────
    ...
    print(f'  Carry-forward: {manual}\n')

    # ── Try CBOE PC ratio (may fail on GitHub Actions) ────────────────────────
    pc_series = pipeline.fetch_cboe_pc()

    # ── Fetch market data for each missing day via individual Ticker calls ────
    fetch_start = (last_valid - pd.Timedelta(days=5)).strftime('%Y-%m-%d')
    fetch_end   = (yesterday  + pd.Timedelta(days=2)).strftime('%Y-%m-%d')

    print(f'  Fetching market data ({fetch_start} → {fetch_end})...')
    ...
    for dt in missing:
        ...
        if np.isnan(spx_val):
            print(f'  {dt.date()}: no SPX data — skipping (holiday?)')
            continue
        ...
        print(f'  {dt.date()}: SPX={spx_val:.2f}  VIX={vix_val:.2f}  '
              f'SPY={spy_val:.2f}  SKEW={skew_val:.2f}  PC={pc_val:.3f}')
    ...
    if appended == 0:
        print('\n  Nothing appended (all missing dates are holidays).')
        return

    # ── Rescore entire history ─────────────────────────────────────────────────
    ...
    print(f'  Latest row: {pd.Timestamp(latest["date"]).date()} | '
          f'SPX={latest["spx"]:.2f} | VIX={latest["vix"]:.2f} | '
          f'MRS={latest["mrs_score"]:+.2f} ({pipeline.regime_label(latest["mrs_score"])})')
    print('\n=== Done ===')
