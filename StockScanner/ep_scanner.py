...
def _batch_download_daily(
    ...
    for i in range(0, len(tickers), BATCH_SIZE):
        ...
        try:
            ...
            # of tickers in a batch without raising an exception. Gap % is
            # computed directly from prev_close, so a stale close here would
            # silently corrupt the flagship EP metric. Re-fetch any ticker
            # lagging the batch's own most-recent date individually.
            if results:
